...
const TimelineItem: FC<{ item: TimelineItem; last?: boolean; onClick?: () => void; isFocused?: boolean; isBlurred?: boolean }> = memo(
  ({ item, onClick, isFocused, isBlurred }) => {
    ...
    return (
      <div className="flex flex-col pb-8 text-center last:pb-0 md:text-left" ref={ref}>
        <motion.div
          ...
          className={`flex flex-col rounded-2xl border border-neutral-300 bg-white p-6 shadow-md transition-all duration-300 hover:shadow-lg cursor-pointer ${isFocused ? 'ring-2 ring-blue-500' : ''
            }`}
          onClick={onClick}
